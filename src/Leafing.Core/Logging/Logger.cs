...
namespace Leafing.Core.Logging {
    public class Logger {
        ...
        private List<ILogRecorder> GetLogRecorders() {
            ...
            foreach (var s1 in ConfigReader.Config.Log.Recorders[_name]) {
                var s2 = s1.Trim();
                if (s2 != "") {
        ...
        protected void Log(LogLevel level, string name, string message, Exception exception) {
            ...
            var configLevel = ConfigReader.Config.Log.Level.ToEnum<LogLevel>();
            if (LogRecorders != null && level <= configLevel) {
