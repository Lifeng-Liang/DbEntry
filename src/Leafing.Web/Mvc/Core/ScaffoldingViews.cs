...
";
        ...
        protected virtual void Process(Action<HtmlBuilder> callback)
        {
            ...
            Process(delegate(HtmlBuilder b)
                    {
                        ...
                        if (itemList != null)
                        {
                            ...
                            foreach (object o in objlist)
                            {
                                ...
                                b.td.include(LinkTo(ControllerName).Title("Show").Action("show").Parameters(id)).end.enter();
                                b.td.include(LinkTo(ControllerName).Title("Edit").Action("edit").Parameters(id)).end.enter();
                                b.td.include(
                                    LinkTo(ControllerName).Title("Destroy").Action("destroy")
                            ...
                            if(_style == ListStyle.Default)
                            {
                                ...
                                {
                                    b.include(LinkTo(ControllerName).Title(i.ToString()).Action("list").Parameters(i)).enter();
                                }
                            }
                            else
                            {
                            ...
                            b.enter().br.br.include(LinkTo(ControllerName).Title("New " + cn).Action("new")).br.enter();
                        }
                    });
        }
            ...
            Process(delegate(HtmlBuilder b)
                    {
                        ...
                        b.include(LinkTo(ControllerName).Title("Edit").Action("edit").Parameters(id)).enter();
                        b.include(LinkTo(ControllerName).Title("Back").Action("list")).enter();
                    });
        }
